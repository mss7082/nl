...
data LineNumberOption = ReverseNumbering | SkipEmptyLines | LeftAlign deriving (Eq)
...
main = do
  ...
  let (mFilePath, options) = parseArguments cliArgs
      numberFunction = if SkipEmptyLines `elem` options then numberNonEmptyLines else numberAllLines
      padMode =
  ...
  maybe (printHelpText "Missing filename") aux mFilePath
